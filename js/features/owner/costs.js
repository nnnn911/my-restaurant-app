...
import { bindSegmentedMonthInputs, getMonthValue, segmentedMonthInput } from '../../ui/datetime.js';
...
const costTotal = (cost = {}) => COST_FIELDS.reduce((sum, [key]) => sum + Number(cost[key] || 0), 0) + Number(cost.staffSalary || 0);

const loadStaffSalary = async () => {
  if (staffSalaryLoaded) return;
  try {
    const actors = await getStaffActors();
    staffSalaryTotal = actors.reduce((sum, actor) => sum + Number(actor.salaryVnd || 0), 0);
  } catch {
    staffSalaryTotal = 0;
  } finally {
...
const filteredCosts = () => {
  ...
  return [...getMonthlyCosts()]
    .filter((cost) => !q || `${cost.month} ${cost.note || ''} ${costTotal(cost)}`.toLowerCase().includes(q))
    .sort((a, b) => {
      const { key, dir } = getSortState(costSort, 'month');
      const direction = dir === 'desc' ? -1 : 1;
      if (key === 'total') return (costTotal(a) - costTotal(b)) * direction;
      if (key === 'staffSalary') return (Number(a.staffSalary || 0) - Number(b.staffSalary || 0)) * direction;
      return (a.month || '').localeCompare(b.month || '') * direction;
    });
};

const costBreakdownRows = (cost = {}) => [
  ...COST_FIELDS.map(([key, label]) => ({ label, value: Number(cost[key] || 0) })),
...
const costChartHtml = ({ type, rows, valueKey, labelKey, height = 280 }) => {
  if (!rows.length) return `<div class="empty-state"><h3>Chưa có dữ liệu</h3></div>`;
...
const mountCostCharts = () => {
  ...
  const palette = [colors.primary, colors.accent, colors.blue, colors.red, colors.purple];
  ...
  pendingCostCharts.forEach((spec) => {
    ...
    const config = spec.type === 'doughnut'
      ? {
          type: 'doughnut',
          data: { labels, datasets: [{ data: values, backgroundColor: labels.map((_, idx) => palette[idx % palette.length]), borderColor: colors.surface, borderWidth: 3 }] },
          options: { ...baseOptions, cutout: '62%' },
        }
      : {
          type: 'bar',
          data: { labels, datasets: [{ data: values, backgroundColor: colors.accent, borderColor: colors.primary, borderWidth: 1, borderRadius: 6 }] },
          options: {
            ...baseOptions,
            plugins: { ...baseOptions.plugins, legend: { display: false } },
            scales: {
              x: { grid: { color: colors.border }, ticks: { color: colors.muted, font: { weight: '600' } } },
              y: { beginAtZero: true, grid: { color: colors.border }, ticks: { color: colors.muted, callback: (value) => formatPrice(Number(value)).replace(/\s?₫$/, '') } },
            },
          },
        };
...
const costBarsHtml = (costs) => {
  ...
  return costChartHtml({ type: 'bar', rows: rows.map((cost) => ({ month: cost.month, total: costTotal(cost) })), valueKey: 'total', labelKey: 'month' });
};

const breakdownBarsHtml = (cost) => {
  const rows = costBreakdownRows(cost);
  return costChartHtml({ type: 'doughnut', rows, valueKey: 'value', labelKey: 'label' });
};
  ...
  return `
    <div class="owner-page owner-analytics-page owner-costs-page">
      ...
      <div class="owner-analytics-grid">
        ...
        <section class="staff-panel"><div class="staff-panel-header"><div class="staff-panel-title">Cơ cấu chi phí ${escapeHtml(latest?.month || '')}</div></div><div class="staff-panel-body">${latest ? breakdownBarsHtml(latest) : `<div class="empty-state"><h3>Chưa có dữ liệu</h3></div>`}</div></section>
      </div>
    </div>
  `;
};
...
const renderCostForm = (cost = null) => `
  <form class="owner-form" id="cost-form">
    <div class="owner-form-grid">
      ...
      ${COST_FIELDS.map(([key, label]) => `<div class="form-group"><label class="form-label" for="cost-${key}">${escapeHtml(label)}</label><input class="form-control" id="cost-${key}" type="number" min="0" step="100000" value="${Number(cost?.[key] || 0)}"></div>`).join('')}
...
const openCostDrawer = (cost = null) => openOwnerDrawer({
...
const bindCostForm = (close, cost = null) => {
  ...
  document.getElementById('cost-form')?.addEventListener('submit', async (e) => {
    ...
    try {
      await saveMonthlyCostOnline({
        ...
        electricity: Number(document.getElementById('cost-electricity')?.value || 0),
        water: Number(document.getElementById('cost-water')?.value || 0),
        rent: Number(document.getElementById('cost-rent')?.value || 0),
        ingredients: Number(document.getElementById('cost-ingredients')?.value || 0),
  ...
  document.getElementById('cost-delete')?.addEventListener('click', async () => {
    if (!cost) return;
    const ok = await openStaffConfirm({ title: 'Xoá chi phí', message: `Xoá chi phí tháng ${cost.month}?`, confirmText: 'Xoá', danger: true });
    if (!ok) return;
...
export const bindCostsPage = () => {
  mountCostCharts();
  document.getElementById('cost-create')?.addEventListener('click', () => openCostDrawer());
  document.querySelectorAll('[data-cost-month]')?.forEach((row) => row.addEventListener('click', () => {
    const cost = getMonthlyCosts().find((item) => item.month === row.dataset.costMonth);
  ...
  document.querySelectorAll('[data-cost-sort]')?.forEach((btn) => btn.addEventListener('click', () => {
