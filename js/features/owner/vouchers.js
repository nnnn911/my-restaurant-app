...
const getSelectedVoucher = () => {
  return getOwnerData().vouchers.find((v) => v.code === voucherSelectedCode) || null;
};

export const renderVoucherPage = () => {
  const q = searchQuery.trim().toLowerCase();
  const vouchers = getOwnerData().vouchers
    .filter((v) => !q || `${v.code || ''} ${v.desc || ''}`.toLowerCase().includes(q))
    .filter((v) => voucherFilter === 'all' || getVoucherStatus(v) === voucherFilter)
    .sort((a, b) => {
      const { key, dir } = getSortState(voucherSort, 'code');
      const direction = dir === 'desc' ? -1 : 1;
      if (key === 'value') return (Number(a.value || 0) - Number(b.value || 0)) * direction;
      if (key === 'minOrder') return (Number(a.minOrder || 0) - Number(b.minOrder || 0)) * direction;
      if (key === 'type') return (a.type || '').localeCompare(b.type || '', 'vi') * direction;
      if (key === 'startsAt') return (a.startsAt || '').localeCompare(b.startsAt || '') * direction;
      if (key === 'expiresAt') return (a.expiresAt || '').localeCompare(b.expiresAt || '') * direction;
      if (key === 'status') return getVoucherStatus(a).localeCompare(getVoucherStatus(b), 'vi') * direction;
      return (a.code || '').localeCompare(b.code || '', 'vi', { numeric: true }) * direction;
    });
  const paged = getPagedRows(vouchers, voucherSheetPage, OWNER_SHEET_PAGE_SIZE);
  voucherSheetPage = paged.page;
  const formatVoucherValue = (v) => v.type === 'percent' ? `${Number(v.value || 0)}%` : formatPrice(Number(v.value || 0));
          ...
          <div class="owner-table-wrap">
            <table class="owner-table owner-spreadsheet owner-voucher-sheet">
              ...
              <tbody>
                ...
                `).join('') || `<tr><td colspan="7"><div class="empty-state"><h3>Không tìm thấy voucher</h3></div></td></tr>`}
              </tbody>
...
const renderVoucherForm = (v) => {
  if (!v) return `<div class="empty-state"><h3>Chọn voucher để chỉnh sửa</h3></div>`;
  return `
...
const bindVoucherForm = (closeDrawer) => {
  ...
  document.getElementById('voucher-form')?.addEventListener('submit', (e) => {
    ...
    if (!code || !Number.isFinite(value) || value < 0 || !Number.isFinite(minOrder) || minOrder < 0) {
    ...
    const next = vouchers.map((v) => v.code === oldCode ? {
      code,
  ...
  document.getElementById('voucher-delete')?.addEventListener('click', async () => {
...
const openCreateVoucherDrawer = () => {
  ...
  document.getElementById('create-voucher-form')?.addEventListener('submit', (e) => {
    ...
    const value = Number(document.getElementById('new-voucher-value')?.value || 0);
    const minOrder = Number(document.getElementById('new-voucher-min')?.value || 0);
    const vouchers = getOwnerData().vouchers;
    if (!code || !Number.isFinite(value) || value < 0 || !Number.isFinite(minOrder) || minOrder < 0) {
      toast.error('Thông tin voucher chưa hợp lệ.');
      return;
...
export const bindVoucherPage = () => {
  ...
  document.getElementById('voucher-new')?.addEventListener('click', openCreateVoucherDrawer);
};
