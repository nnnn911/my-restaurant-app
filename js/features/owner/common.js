...
export const moneyInput = (id, value, attrs = "") => `
  <div class="owner-money-input">
    <input class="form-control" id="${id}" type="number" min="0" step="1" value="${Number(value || 0)}" ${attrs}>
    <span>VND</span>
  </div>
`;
...
export const openOwnerDrawer = ({
  id = "owner-object-drawer",
  title,
  label,
  bodyHtml,
  onBind,
  ...
  drawer
    .querySelectorAll("[data-owner-drawer-close]")
...
export const getSortState = (value = "", fallback = "id") => {
  ...
  return match
    ? { key: match[1], dir: match[2] }
    : { key: value || fallback, dir: "asc" };
};
...
export const ownerPaginationHtml = ({
  total,
  page,
  pageCount,
  label,
...
export const setOwnerRenderPage = (fn) => {
  ownerRenderPage = fn;
};
export const rerenderOwnerPage = () => {
  if (typeof ownerRenderPage === "function") ownerRenderPage();
};
